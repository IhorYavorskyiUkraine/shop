...
export const ASideCatalog: React.FC<Props> = ({ listToRender }) => {
   ...
   const handleTabClick = (i: number) => {
      dispatch(toggleTab(i));
   };

   const handleCategory = (i: number, category: string) => {
      handleTabClick(i);
      dispatch(setCategory(category.toLowerCase()));
   };

   const debouncedSetPrice = debounce((value: number[]) => {
      dispatch(setPrice(value));
   }, 300);

   const handleColor = (color: string, i: number) => {
      dispatch(setActiveIndex(i));
      dispatch(setColor(color.toLowerCase()));
   };

   const handleSize = (size: string) => {
      dispatch(setSize(size.toLowerCase()));
   };

   const handleDressStyle = (dressStyle: string) => {
      dispatch(setDressStyle(dressStyle.toLowerCase()));
   };
      ...
      <aside className={styles.wrapper}>
         ...
         <div className={styles.categories}>
            {categories.map((item, i) => (
               <button key={i} onClick={() => handleCategory(i, item)}>
                  {item}
         ...
         <div className={styles.tab}>
            <div onClick={() => handleTabClick(categories.length)}>
               <h4>Price</h4>
               <img
                  src={arrow}
                  style={
                     activeTabs[categories.length]
         ...
         <div className={styles.tab}>
            <div onClick={() => handleTabClick(categories.length + 1)}>
               <h4>Colors</h4>
               <img
                  src={arrow}
                  style={
                     activeTabs[categories.length + 1]
                        ? { transform: "rotate(180deg)" }
                        : {}
                  }
                  alt="arrow"
                        ...
                        {i === activeIndex && (
                           <span
                              className={`${styles.checkmark} ${
                                 item === "white" ? styles.checkmarkBlack : ""
                              }`}
                           ></span>
                        )}
                     </button>
         ...
         <div className={styles.tab}>
            <div onClick={() => handleTabClick(categories.length + 2)}>
               <h4>Size</h4>
               <img
                  src={arrow}
                  style={
                     activeTabs[categories.length + 2]
                        ? { transform: "rotate(180deg)" }
                        : {}
                  }
                  alt="arrow"
                     ...
                     return (
                        <button
                           className={`${styles.sizeButton} ${
                              item.toLowerCase() === size ? styles.active : ""
                           }`}
         ...
         <div className={styles.tab}>
            <div onClick={() => handleTabClick(categories.length + 3)}>
               <h4>Dress Style</h4>
               <img
                  src={arrow}
                  style={
                     activeTabs[categories.length + 3]
                        ? { transform: "rotate(180deg)" }
                        : {}
                  }
                  alt="arrow"
         ...
         <Button text="Apply Filter" className="apply" onClick={applyFilters} />
      </aside>
   );
};
